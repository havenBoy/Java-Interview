...
- **原理**
  ...
  - 公平与非公平：是指线程获取锁的方式，默认是非公平
    ...
    - 性能问题的比较：
      ...
      按道理来说，有序的进行要比无序的速率要快，不然公众场合也不会要排队，但是为什么这里的非公平锁要比公平锁要快呢？是因为假如线程A已经在持有锁，线程B去竞争，假如A没有释放，那么线程B会被挂起，此时如果线程A释放了锁，同时线程C也来竞争锁，那么首先回去对线程B进行唤醒，但是也许在唤醒线程B的同事，线程C早已获得锁，并且在时间的间隙已经使用完，那么这时系统的吞吐量就提高了；
