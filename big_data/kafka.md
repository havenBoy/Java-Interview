...
零拷贝技术减少了拷贝的次数，传统需要经过用户态-》内核态的切换，而零拷贝将从磁盘直接到内核态

较少了上下文切换与资源竞争的问题

2. 文件顺序写 

磁盘的顺序写大多数情况下比随机内存写还要快
...
何时会触发rebalance？ 消费组的消费实例发生变化/订阅主题的数量发生变化/主题的分区数发生变化
...
- kafka使用磁盘的顺序读与零拷贝技术，使得主副本与从副本的消息差量变得更少

#### leader发生的宕机，ISR也不存在，发生什么？
