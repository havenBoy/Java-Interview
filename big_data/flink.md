...
- ##### 作业管理器   jobmanager
  ...
  - 是最核心的组件，负责处理单独作业的资源请求
  - 在作业提交时，此对象会接受到需要提交的应用，包括了jar包，数据流图以及作业图
  - 收集作业的状态以及taskmanager的管理
...
- ##### 任务与任务槽
  ...
  taskslots:  对于每一个taskmanager节点上可以并行执行的任务数，每个任务槽会拥有taskManager的一部分资源用来执行一个独立的子任务的

  设置：

  在配置文件中，使用参数taskmanager.numOfTaskSlots 指定
...
有状态流应用中的检查点（checkpoint），其实就是所有任务的状态在某个时间点的一个快照

状态后端：

jobmanager发布checkpoint的指令，由taskmanager来执行，完成后汇报给jobmanager

存储介质包含：rockdb、filesystem、memory， 即包含hdfs的checkpoint保存

故障重启策略：（在开启checkpoint后才会生效）
...
1. source   支持数据重放
2. flink 内部通过checkpoint机制保证，是一种分布式的快照，保证作业出现失败后可以从最新快照恢复
3. sink端故障恢复时，数据不会重复写入外部系统
4. 核心思想是等到chekpoint真正完成时，才会将所有的结果写入到sink系统
5. 对于每个checkpoint，会开启一个事务，在将数据写入外部sink时，只是预提交
6. 当chekpoint真正写入完毕后，才会真正提交事务，但要求是sink端能够支持事务，是二阶段提交的体现

#### flink时间

1. 事件时间，一半是事件本身带有的属性值
2. 处理时间，事件被flink算子处理的真正时间
3. 摄入时间，指的是事件进入flink的时间，一般不太使用

#### flink水印

用来衡量事件时间进展的机制，用来处理乱序事件的，通常会结合窗口来实现，

表示事件时间小于watermak的事件都到达了，可以进行处理了

在实现上，需要有一个时间提取器提取事件的发生时间，水印代表
