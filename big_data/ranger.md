...
>  安装节点为集群内的任意节点，建议为主节点  
  1. 解压编译打包完成后target目录下ranger-2.1.0-admin.tar.gz文件到某个路径下。  
     tar -zxvf ranger-2.1.0-admin.tar.gz -C /opt/
  2. 修改install.properties文件内容，内容如下：  
  ```
  ...
  ```
  3. 赋予数据库用户root用户在localhost与当前节点管理网地址的所有权限，原因是需要使用root用户创建关于ranger数据库，如下示例：
  ```sql
  flush privileges;
  ...
  flush privileges;
  ```
  4. 初始化服务，执行命令./setup.sh
  5. 启动ranger-admin，执行命令ranger-admin start 
  6. 界面输入policymgr_external_url中设置的url，进行登录ranger界面，默认账户/密码为admin/admin
